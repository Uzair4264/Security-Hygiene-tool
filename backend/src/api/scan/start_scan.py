...
def run_local_scan(scan_id: str, user_id: str, target: str, scan_type: str):
    """Run scan orchestration locally in a background thread."""
    time.sleep(1)
    try:
        ...
        logger.error("Local scan execution failed", scan_id=scan_id, error=str(e))
    ...
    try:
        ...
        if os.getenv("LOCAL_MODE") == "true":
            ...
            thread = threading.Thread(
                ...
                args=(scan_id, user_id, scan_request.target, scan_request.scan_type.value),
                daemon=True
            )
            thread.start()
        else:
            try:
                ...
                logger.error("Failed to invoke scan processor", scan_id=scan_id, error=str(e))
